...
host-side engine. The Monero side and the maker/taker daemon are not built yet, so the
end-to-end protocol's safety also depends on code that doesn't exist here yet.
